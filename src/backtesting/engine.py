...
class BacktestEngine:
    ...
    def __init__(
        ...
        self.min_indicator_alignment = (
            min_indicator_alignment or config.model.MIN_INDICATOR_ALIGNMENT
        )
    ...
    def run(
        ...
        if df.empty or len(df) < self.lookback + 10:
            ...
            return BacktestResult(
                symbol=symbol, total_trades=0, win_trades=0, loss_trades=0,
                win_rate=0.0, total_return_pct=0.0, profit_factor=0.0,
        ...
        for i in range(self.lookback, len(df)):
            ...
            aligned = (
                signals.get("bull_count", 0) if ml_dir == "UP"
                else signals.get("bear_count", 0)
            )
